...
#include <ctime>
using namespace std;
void createObj();void downloadIMG(string imgSRC, int sd);void downloadHrefHtm(string href, int sd);
...
void downloadHrefOther(string href, int sd) {
    // 切割imgSRC -> 得到domain&path
    ParseUrl(href);
...
void downloadHrefHtm(string href, int sd) {
    ParseUrl(href);
    printf("-- 物件%d --\n", fileNum);
    string req = "";
    char buf[10000]={};
